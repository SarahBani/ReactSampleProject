export const FETCH_HOTELS = 'FETCH_HOTELS';
export const SET_HOTELS = 'SET_HOTELS';
export const FETCH_HOTELS_COUNT = 'FETCH_HOTELS_COUNT';
export const SET_HOTELS_COUNT = 'SET_HOTELS_COUNT';
export const FETCH_HOTEL = 'FETCH_HOTEL';
export const SET_HOTEL = 'SET_HOTEL';
export const CLEAR_SELECTED_HOTEL = 'CLEAR_SELECTED_HOTEL';
export const FETCH_HOTEL_PHOTOS = 'FETCH_HOTEL_PHOTOS';
export const SET_HOTEL_PHOTOS = 'SET_HOTEL_PHOTOS';
export const SAVE_HOTEL = 'SAVE_HOTEL';
export const DELETE_HOTEL = 'DELETE_HOTEL';
export const UPLOAD_HOTEL_PHOTO = 'UPLOAD_HOTEL_PHOTO';
export const REMOVE_HOTEL_PHOTO = 'REMOVE_HOTEL_PHOTO';
export const SAVE_HOTEL_PHOTO = 'SAVE_HOTEL_PHOTO';
export const DELETE_HOTEL_PHOTO = 'DELETE_HOTEL_PHOTO';